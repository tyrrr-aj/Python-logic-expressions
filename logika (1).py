...
def Quine_McCluskey(expression): # shortens the expression using the Quine - McCluskey's algorithm
    ...
    def essential_conjunctions(reps, store): # finds essential representations and moves them from store to result
        result = []
        included = set()
        for e in reps: # we check each of them to find out if it only occurs in one term - such term would become essential
            occured = False
            for i, k in enumerate(store):
                if e in k[0]:
                    if occured: # representation occures more than once - it doesen't make any term essential
                        break
                    else:
                        occured = True
                        spot = i
            else:
                if occured: # representation occured only once - it's essential
                    tmp = store.pop(spot) # essential representation is removed form stack of candidates
                    included.update(tmp[0]) # we store terms that are already included in result
                    for k in store: # terms that are included in result no longer matter for representations still on store
                        k[0].difference_update(included)
                    result.append(tmp[1]) # essential representation is added to result
        new_store = []
        for e in store: # clean-up on store - if some reps are left with no terms assigned, they won't be necessary, so we omit them
            if e[0]:
                new_store.append(e)
        unincluded = reps - included
        return (result, unincluded, new_store)
    
    def Petricks_method(store, unincluded): # chooses shortest possible set of non-essential representations covering all the unused terms
        from copy import deepcopy
        
        def var_number(conjunctions):
            count = 0
        ...
        columns = {} # brackets made of columns in Petrick's algorithm
        for e in unincluded:
            columns[e] = []
        for e in store: # we iterate through all representations
            for k in e[0]: # adding them to each column corresponding with terms that they are assigned to
                columns[k].append(e[1])
        print(columns)
        alternatives = [{e} for e in columns.popitem()[1]]
        print(alternatives)
        print(columns)
        for e in columns.values():
            ...
            for f in e:
                for g in alternatives:
                    new_alternatives.append(g | {f})
            alternatives = deepcopy(new_alternatives)
        print(alternatives)
        min_con = [alternatives.pop()]
        for e in alternatives:
            if len(e) < len(min_con[0]):
                min_con = [e]
            elif len(e) == len(min_con[0]):
                min_con.append(e)
        print(min_con)
        min_vars = min_con[0]
        min_vars_size = var_number(min_vars)
        for e in min_con:
            if var_number(e) < min_vars_size:
                min_vars = e
                min_vars_size = var_number(e)
        return min_vars
    ...
    result, unincluded, store = essential_conjunctions(reps, store)
    if store:
        result.extend(Petricks_method(store, unincluded))
